...
class Particle(namedtuple("Particle", ["ID", "molecule_ID", "type", "x", "y", "z", "charge"])):
    ...
    def rotate(self, yaw=0, pitch=0, roll=0, shift_x=0, shift_y=0, shift_z=0):
        Rx = np.mat([[1,0,0],[0,np.cos(yaw),-np.sin(yaw)],[0,np.sin(yaw),np.cos(yaw)]])
        Ry = np.mat([[np.cos(pitch),0,np.sin(pitch)],[0,1,0],[-np.sin(pitch),0,np.cos(pitch)]])
        Rz = np.mat([[np.cos(roll),-np.sin(roll),0],[np.sin(roll),np.cos(roll),0],[0,0,1]])
        [_x_new], [_y_new], [_z_new] = np.array(Rz * Ry * Rx * np.mat([[self.x - shift_x],[self.y - shift_y],[self.z - shift_z]]))
...
class Types:
    class Atom_Type:
        def __init__(self, name, ID, **args):
            ...
            self.mass = args.get('mass', str(45 / 72) if name[0] == 'S' else 1)
        ...
        for i in range(1, len(self.beads) + 1):
            for j in range(i, len(self.beads) + 1):
                ...
                if (p1.solid or p2.solid) and (not (p1.virtual or p2.virtual)):
                    #result += f"pair_coeff {i} {j} dpd 1000 0 {str(p1.Rc(p2) * 0.860254013828)} # {p1.name} {p2.name}\n"
                    result += f"pair_coeff {i} {j} dpd 1000 0 {str(p1.Rc(p2) * 0.9)} # {p1.name} {p2.name}\n"
        return result
...
class Data:
    ...
    def fill_water(self, ion_concentration=0.15, solid_volume = 0.0, factor = 5.0, factor_rho = 1.0):
        ...
        CL_counter = 1
        W_counter = 1
        n_remaining_K = N_ion_pairs
        n_remaining_CL = N_ion_pairs
        ...
        for i in range(int(n_remaining_K)):
            skip = False
            x = np.random.uniform(-self.x_len / 2, self.x_len / 2)
            y = np.random.uniform(-self.y_len / 2, self.y_len / 2)
            z = np.random.uniform(-self.z_len / 2, self.z_len / 2)
            ...
            if not skip:
                self.add_particle(f"K{K_counter}", "K", x, y, z, 1, "solution")
                K_counter += 1
        for i in range(int(n_remaining_CL)):
            skip = False
            x = np.random.uniform(-self.x_len / 2, self.x_len / 2)
            y = np.random.uniform(-self.y_len / 2, self.y_len / 2)
            z = np.random.uniform(-self.z_len / 2, self.z_len / 2)
