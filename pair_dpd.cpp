  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsq[itype][jtype]) {
        ...
        if (eflag) {
          evdwl = 0.5*a0[itype][jtype]*cut[itype][jtype] * wd*wd - 30.885561677818*pow((1.0-r/1.183899254088), 2);
          evdwl *= factor_dpd;
        }

        if (evflag) ev_tally(i,j,nlocal,newton_pair,
                             evdwl,0.0,fpair,delx,dely,delz);
      } else if (rsq < pow(1.183899254088, 2)) {
        if (eflag) {
          r = sqrt(rsq);
          //evdwl = - 37.094920551378*(1.0-r/1.19489090480136)*(1.0-r/1.19489090480136);
          //evdwl = - 27.7949*(1.0-r/1.2819)*(1.0-r/1.2819);
          evdwl = - 30.885561677818*pow((1.0-r/1.183899254088), 2);
          evdwl *= factor_dpd;
        }
        if (evflag) ev_tally(i,j,nlocal,newton_pair,
                             evdwl,0.0,0.0,delx,dely,delz);
      }
    }
  }
                       ...
                       double /*factor_coul*/, double factor_dpd, double &fforce)
{
  ...
  phi = 0.5*a0[itype][jtype]*cut[itype][jtype] * wd*wd - 30.885561677818*pow((1.0-r/1.183899254088), 2);
